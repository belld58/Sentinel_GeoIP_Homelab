...
# We are using this as filter specifically to exclusively parse out failed RDP authentication attempts from our Windows Event Viewer.
$XMLFilter = @'
<QueryList> 
...
'@
<#
    For starters, we need to train the Extract feature in Azure's Log Analytics Workspace with this
    log sample so that it fully extracts all the specific fields we need with minimal error.
    Of course, we will be filtering out these "fakehost" destination records from the map in our logs.
#>
